  ...
  <div class="container">
    <div class="toppanel-header">
      <div class="col-md-4 no-padding">
        <p class="breadcum">Home / <a href="#">News </a> / Publication / Publication Detail / <?php echo $publication_detail[0]['title']; ?></p>
      </div>
  ...
  <div class="col-md-8 panel-right paddingL20">
    <div class="rows">
      <div class="col-md-7">
        ...
        <?php //foreach ($publication_detail as $key => $value) { ?>
        <div class="col-md-12 no-padding margin-bottom-10">
          <div class="paragraph">
            <img src="<?php echo HTTP_IMAGE_UPLOADED_PATH . $publication_detail[0]['image']; ?>" class="img-responsive width-full margin-bottom-20" alt="">
            <h2 class="head-date"><?php echo $publication_detail[0]['title']; ?></h2>
            <h3 class="head-subline-date">
              <?php
                if($publication_detail[0]['date_published'] != '0000-00-00 00:00:00'){
                    echo date('d F Y', strtotime($publication_detail[0]['date_published']));
                }else{
                    echo substr($publication_detail[0]['date_published'],0,10);
                }
              ?>
            </h3>
            ...
            <?php
                $fbslug = '';
                $fburl  = '';
                if(!empty($publication_detail[0]['slug'])){
                  $fbslug = $publication_detail[0]['slug'];
                  $fburl  = base_url() . 'publication/publication_detail/' . $fbslug;
                }else{
                  $fbslug = $publication_detail[0]['id'];
                  $fburl  = base_url() . 'publication/publication_detail/' . $fbslug;
                };
                $titlefb=urlencode($publication_detail[0]['title']);
                $urlfb=urlencode($fburl);
                $summaryfb=urlencode($publication_detail[0]['content']);
                $imagepath = '';
                //if(!empty($image)){ $imagepath = HTTP_IMAGE_UPLOADED_PATH . $comic_detail[0]; } else { $imagepath =  HTTP_IMAGES_PATH . 'default.png';}
               if(!empty($publication_detail[0]['image'])){
                    $imagepath = HTTP_IMAGE_UPLOADED_PATH . $publication_detail[0]['image'];
            ...
            <div class="col-md-12">
            <?php
                $content = $publication_detail[0]['content'];
          ...
          <div class="col-md-12">
            <ul class="list-style-none list-comic-date">
        			<?php
        			    ...
        			    foreach($other_publication['other_publication'] as $key => $value){?>
        				  <li> <a href="<?php echo base_url() . 'publication/publication_detail/' . $value['slug']?>"><b><?php echo $value['title']; ?></b></a><br>
                          <?php
