...
class Comics extends CI_Controller {
    ...
    public function all() {
        ...
        $totalRec = count($this->comic->getRows(array('country' => $country, 'genre' => $genre, 'category' => $category)));

        $config = array();
        //pagination configuration

        $config['div']         = 'comicList'; //parent div tag id
        $config["base_url"]    = base_url().'comics/ajaxPaginationData';
        ...
        //get the posts data
        $arr['comic'] = $this->comic->getRows(array('limit'=>$this->perPage, 'country' => $country, 'genre' => $genre, 'category' => $category));
    ...
    function ajaxPaginationData(){
        ...
        //get the posts data
        $arr['comic'] = $this->comic->getRows(array('start'=>$page,'limit'=>$this->perPage, 'country' => $country, 'genre' => $genre, 'category' => $category));
    ...
    public function ecomics(){
        ...
        $config = array();
        $config["base_url"] = base_url() . "comics/ecomics";
        $config["total_rows"] = $this->record_count_ecomics();
        $config["per_page"] = 16;
        $config["uri_segment"] = 3;
        $choice = $config["total_rows"] / $config["per_page"];
        $config["num_links"] = 3;
        ...
        $arr['ecomics'] = $this->fetch_ecomics($config["per_page"], $page);
        $str_links = $this->pagination->create_links();
        $arr["links"] = explode('&nbsp;',$str_links );
        $arr['sosmed'] = $this->getSosmed();
        $arr['logo'] = $this->getLogo();
