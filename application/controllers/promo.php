...
class Promo extends CI_Controller {
    ...
    public function all() {
      ...
      $category = $this->getCategoryIdByName('Latest Promo');
      ...
      $config["base_url"]    = base_url().'promo/ajaxPaginationData/';
      ...
      //get the posts data
      $arr['promo'] = $this->postpromo->getRows(array('limit'=>$this->perPage, 'category' => $category));

      $arr['content'] = '';
      $this->load->view('vwHeader',$arr);
      $this->load->view('vwPromo',$arr);
    }
    ...
    function ajaxPaginationData(){
        ...
        //get the posts data
        $arr['promo'] = $this->postpromo->getRows(array('start'=>$page,'limit'=>$this->perPage, 'category' => $category));

        //load the view
        $this->load->view('vwPromoPagination', $arr, false);
    }
    ...
    public function promo_detail($slug){
        ...
        if(!empty($d)){
          $arr['promo_detail'] = $this->db->query($qry)->result_array();
        }else{
          redirect('blankpage');
        }
        ...
        $arr['twitter'] = $this->getTwitter();
    		...
    		$arr['left_banner'] = $this->getLeftBanner();
    		$arr['right_banner'] = $this->getRightBanner();
    		$arr['other_promo'] = $this->getOtherPromo();

        $arr['page'] = $arr['promo_detail'][0]['title'];
        $arr['content'] = substr($arr['promo_detail'][0]['content'], 0, 450);
        $this->load->view('vwHeader',$arr);

        $this->load->view('vwPromoDetail',$arr);
    }
